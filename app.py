...
from utils import NLPUtils, _ensure_nltk_resources
import asyncio

# Initialize NLTK resources at app startup
@st.cache_resource
def initialize_nlp_resources():
    """Initialize all NLP resources with caching"""
    try:
        success = _ensure_nltk_resources()
        if success:
            st.success("✅ NLP resources initialized successfully!")
        else:
            st.warning("⚠️ Some NLP resources may not be available")
        return success
    except Exception as e:
        st.error(f"❌ Error initializing NLP resources: {e}")
        return False
